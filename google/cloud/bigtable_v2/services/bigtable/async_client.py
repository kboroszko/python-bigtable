    ...
    def execute_query(
        ...
        *,
        ...
        Args:
            ...
            instance_name (str):
                Required. The unique name of the instance from which to
                read. Values are of the form
        ...
        # Quick check: If we got a request object, we should *not* have
        # gotten any keyword arguments that map to the request.
        has_flattened_params = any([instance_name, app_profile_id])
        if request is not None and has_flattened_params:
            raise ValueError(
                "If the `request` argument is set, then none of "
                "the individual field arguments should be set."
        ...
        routing_param_regex = re.compile(
            "^(?P<instance_name>projects/[^/]+/instances/[^/]+)$"
        )
        regex_match = routing_param_regex.match(request.instance_name)
        if regex_match and regex_match.group("instance_name"):
            header_params["instance_name"] = regex_match.group("instance_name")
